...
class Conv2D(object):
    def __init__(self, shape, output_channels, ksize=3, stride=1, method='VALID'):
        ...
        self.weights = np.random.randn(ksize, ksize, self.input_channels, self.output_channels) / weights_scale
        self.bias = np.random.randn(self.output_channels) / weights_scale
        if method == 'VALID':
    ...
    def forward(self, x):
        ...
        self.col_image = []
        outshape=(x.shape[0],self.eta.shape[1],self.eta.shape[2],self.eta.shape[3])
        conv_out = np.zeros(outshape)
...
if __name__ == "__main__":
    ...
    cv2.imwrite('out.jpg',next[0])
    next1 = next.copy() + 1
    conv.gradient(next1-next)
    print (conv.w_gradient)
    print (conv.b_gradient.shape)
    conv.backward()
    print(conv.w_gradient)
    print(conv.b_gradient.shape)